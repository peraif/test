...
const Header = () => {
  ...
  useEffect(() => {
    window.addEventListener('scroll', () => {
      ...
      const navbarHeight = ref.current?.offsetHeight;
      if (navbarHeight && windowY > navbarHeight)
        ref.current?.classList.add(s['header__show-border']);
    ...
    <header ref={ref} className={s.header}>
      <Container>
        <div className={s['header__items']}>
          ...
          <BasicLink href={`/${Paths.shopping_cart}`} className={s['header__shopping-link']}>
            Shopping Cart{' '}
            <Image width={20} height={20} src="/images/shopping-cart.svg" alt="shopping-cart" />
          </BasicLink>
        </div>
      </Container>
    </header>
