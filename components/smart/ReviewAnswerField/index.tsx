...
const ReviewAnswerField = ({id, name, answerChildren, accountName}: ReviewAnswerFieldProps) => {
    ...
    return (
        <>
            {!statusAnswerField && (<Button thumbnail variant={"text"} onClick={showAnswerField}>Ответить</Button>)}
            {statusAnswerField && (
                <>
                    <Input
                    ...
                    <div className={s["review-input-field__buttons"]}>
                        <Button thumbnail variant={"text"} onClick={hideAnswerField}>Отмена</Button>
                        <Button thumbnail variant={"text"} onClick={addNewAnswer}>Ответить</Button>
                    </div>
                </>
            )}
        </>
