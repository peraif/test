...
const ProductPageItem = ({ item }: ProductPageItemProps) => {
  ...
  const countProductInCart = useMemo(
    () => cartItems?.find((elem) => elem.id === item?.id),
    [cartItems, item]
  );
  ...
  const openReviews = () => {
    if (!query.param || (query.param.length > 1 && query.param?.[1].includes(Paths.reviews)))
      return;
  ...
  return (
    <div className={s['product-page-item']}>
      ...
      <div className={s['product-page-item__block']}>
        <ProductCategory className={s['product-page-item__category']}>
          {item.category}
        </ProductCategory>
        <div className={s['product-page-item__ratings-block']}>
          <ProductRating productId={item.id} ratingCount={item.rating_count} />
          {item.reviews && item.reviews.length > 0 && (
            <ProductReviewsCount onClick={openReviews} reviewsCount={item.reviews.length} />
          )}
        </div>
      </div>
      <ProductText
        onClick={openProductPage}
        className={s['product-page-item__text']}
        text={item.text}
      />
      <ProductPrice price={item.price} measurementName={'шт.'} prefix={'₽'} />
      <div className={s['product-page-item__block']}>
        ...
        {!asPath.includes(Paths.shopping_cart) && (
